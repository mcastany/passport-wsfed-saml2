...
// Assertion // Subject
SAML.prototype.validateRecipent = function(samlAssertion, destination){
  var subjectConfirmationData = xpath.select("//*[local-name(.)='Subject']/*[local-name(.)='SubjectConfirmationData']", samlAssertion);
  
  // Values is optional. Only validate if found
  if (!subjectConfirmationData || subjectConfirmationData.length === 0) return false;

  var recipent = subjectConfirmationData.getAttribute('Recipient');
  if (!recipent) return false;

  return recipent && recipent === destination;
};
...
SAML.prototype.parseAssertion = function(samlAssertion, callback) {
  ...
  if (self.options.checkRecipent && !self.validateRecipent(samlAssertion, self.options.identityProviderUrl)) {
    return callback(new Error('Recipent is invalid. Configured: ' + self.options.identityProviderUrl), null);
  }
