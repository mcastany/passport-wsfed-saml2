...
Samlp.prototype = {
  ...
  validateSamlResponse: function (samlResponse, callback) {
    ...
    if (inResponseTo && self.options.request_id && inResponseTo !== self.options.request_id){ 
      return callback(new Error('InResponseTo attribute does not match the id in the AuthNRequest'));
    }
