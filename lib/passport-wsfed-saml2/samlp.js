...
Samlp.prototype = {
  ...
  validateSamlResponse: function (samlResponse, options, callback) {
    ...
    // validate reponse before checking saml status
    var validationErrors = validateSamlResponse(samlResponse, options);
    if (validationErrors){
...
function validateSamlResponse(samlResponse, options){
  // Ignore validations if there are no settings
  if (!options) return;
  ...
  // Check reponse
  var responseXML = xpath.select("//*[local-name(.)='Reponse']", samlResponse)[0];
  if (!response) return new Error('XML is not a valid saml response');



  var inResponseTo = statusCodeXml.getAttribute('InResponseTo');

  if (inResponseTo && inResponseTo !== options.requestID){
    return new Error('Response doesn\'t match any request');
  }

  var destination = statusCodeXml.getAttribute('Destination');
  if (destination && destination !== options.destinationUrl){
    return new Error('Destination does not match');    
  }
