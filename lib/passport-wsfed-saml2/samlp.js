...
Samlp.prototype = {
  getSamlRequestParams: function (opts, callback) {
    ...
    var model = {
      ID:               options._id || '_' + generateUniqueID(),
  ...
  validateSamlResponse: function (samlResponse, options, callback) {
    ...
    var validationErrors = validateResponse(samlResponse, options);
    if (validationErrors){
...
function validateResponse(samlResponse, options){
  ...
  if (!responseXML) return new Error('XML is not a valid saml response');


  var inResponseTo = responseXML.getAttribute('InResponseTo');

  if (inResponseTo && options.request_id && inResponseTo !== options.request_id){
    return new Error('Response doesn\'t match any request');
  }

  var destination = responseXML.getAttribute('Destination');
  if (destination && destination !== options.identityProviderUrl){
