...
function WsFedSaml2Strategy (options, verify) {
  ...
  this.options.eventEmitter = new EventEmitter();
  ...
  this._extraValidation = options.extraValidation && typeof options.extraValidation === 'function' ? options.extraValidation : extraValidation
...
function extraValidation(samlRespose, callback){
  return callback(); 
}
...
WsFedSaml2Strategy.prototype._authenticate_saml = function (req, state) {
  ...
  self._wsfed.retrieveToken(req, function(err, token) {
    ...
    self.options.recipientUrl = self.options.recipientUrl || utils.getReqUrl(req);
  ...
  function executeSamlp(req) {
    if (req.body && req.method === 'POST' && req.body.SAMLResponse) {
      ...
      var loaded = function (err, ok, state) {
        ...
        var req_full_url = utils.getReqUrl(req);
        ...
        self._samlp.validateSamlResponse(samlResponseDom, function (err, profile) {                
          ...
          self._extraValidation(samlResponseDom, function(err){
            if (err) return self.fail(err, err.status || 400);
